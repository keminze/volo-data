...
class AzureAISearch_VectorStore(VannaBase):
    ...
    def _create_index(self) -> bool:
        fields = [
            ...
            SearchableField(
                name="document", type=SearchFieldDataType.String, searchable=True, filterable=True
            ),
            SearchField(
                name="type", type=SearchFieldDataType.String, filterable=True, searchable=True
            ),
            SearchField(
    ...
    def get_training_data(self) -> List[str]:
        ...
        if len(df):
            df.loc[df["type"] == "sql", "question"] = df.loc[df["type"] == "sql"]["document"].apply(
                lambda x: json.loads(x)["question"]
            )
            df.loc[df["type"] == "sql", "content"] = df.loc[df["type"] == "sql"]["document"].apply(
                lambda x: json.loads(x)["sql"]
            )
