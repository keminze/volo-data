...
import pandas as pd
from qdrant_client import QdrantClient, grpc, models, AsyncQdrantClient
...
class Qdrant_VectorStore(VannaBase):
    ...
    def __init__(
        ...
        if client is None:
            ...
            raise TypeError(f"Unsupported client of type {client.__class__} was set in config")
    ...
    def get_training_data(self, **kwargs) -> pd.DataFrame:
        ...
        if sql_data := self._get_all_points(self.sql_collection_name):
            ...
            id_list = [
                self._format_point_id(data.id, self.sql_collection_name) for data in sql_data
            ]
        ...
        if ddl_data := self._get_all_points(self.ddl_collection_name):
            ...
            id_list = [
                self._format_point_id(data.id, self.ddl_collection_name) for data in ddl_data
            ]
    ...
    def generate_embedding(self, data: str, **kwargs) -> List[float]:
        embedding_model = self._client._get_or_init_model(
            model_name=self.fastembed_model, cache_dir="./huggingface_cache"
        )
...
# class AsyncQdrant_VectorStore(VannaBase):
#     """
#     Vectorstore implementation using Qdrant - https://qdrant.tech/
...
#                 prefix=config.get("prefix", None),
#             )
#         elif not isinstance(client, AsyncQdrantClient):
#             raise TypeError(
...
#                 models.PointStruct(
#                     id=id,
#                     vector= await self.generate_embedding(question_answer),
#                     payload={
#                         "question": question,
#                         "sql": sql,
#                     },
...
#                 models.PointStruct(
#                     id=id,
#                     vector= await self.generate_embedding(ddl),
#                     payload={
#                         "ddl": ddl,
#                     },
#                 )
...
#                 models.PointStruct(
#                     id=id,
#                     vector= await self.generate_embedding(documentation),
#                     payload={
#                         "documentation": documentation,
#                     },
#                 )
...
#         if sql_data := await self._get_all_points(self.sql_collection_name):
#             question_list = [data.payload["question"] for data in sql_data]
#             sql_list = [data.payload["sql"] for data in sql_data]
#             id_list = [
...
#         if ddl_data := await self._get_all_points(self.ddl_collection_name):
#             ddl_list = [data.payload["ddl"] for data in ddl_data]
#             id_list = [
...
#         if doc_data := await self._get_all_points(self.documentation_collection_name):
#             document_list = [data.payload["documentation"] for data in doc_data]
#             id_list = [
#                 self._format_point_id(data.id, self.documentation_collection_name)
#                 for data in doc_data
...
#         try:
#             id, collection_name = self._parse_point_id(id)
#             res = await self._client.delete(collection_name, points_selector=[id])
#             return True
#         except ValueError:
#             return False
...
#     async def generate_embedding(self, data):
#         embedding_model = await self._client._get_or_init_model(
#             model_name=self.fastembed_model
#         )
#         # 放线程池里避免阻塞
#         embedding = await asyncio.to_thread(lambda: next(embedding_model.embed(data)))
#         return embedding.tolist()

#     async def get_similar_question_sql(self, question: str, **kwargs) -> list:
#         results = await self._client.query_points(
#             self.sql_collection_name,
#             query=await self.generate_embedding(question),
#             limit=self.n_results,
#             with_payload=True,
#         ).points
...
#     async def _get_all_points(self, collection_name: str):
#         results: List[models.Record] = []
#         next_offset = None
#         stop_scrolling = False
#         while not stop_scrolling:
...
#         if not await self._client.collection_exists(self.sql_collection_name):
...
#         if not await self._client.collection_exists(self.ddl_collection_name):
#             await self._client.create_collection(
#                 collection_name=self.ddl_collection_name,
#                 vectors_config=models.VectorParams(
#                     size=self.embeddings_dimension,
...
#                 **self.collection_params,
#             )
#         if not await self._client.collection_exists(self.documentation_collection_name):
#             await self._client.create_collection(
#                 collection_name=self.documentation_collection_name,
#                 vectors_config=models.VectorParams(
#                     size=self.embeddings_dimension,
