...
class VannaDB_VectorStore(VannaBase, VannaAdvanced):
    ...
    def update_function(self, old_function_name: str, updated_function: dict) -> bool:
        ...
        def validate_arguments(args):
            return [{key: arg[key] for key in arg if key in ArgumentKeys} for arg in args]

        # Keep only the keys that conform to the SQLFunctionUpdate GraphQL input type
        updated_function = {
            key: value for key, value in updated_function.items() if key in SQLFunctionUpdate
        }
        ...
        variables = {"input": {"old_function_name": old_function_name, **updated_function}}
