...
class FAISS(VannaBase):
    ...
    def generate_embedding(self, data: str, **kwargs) -> List[float]:
        ...
        assert (
            embedding.shape[0] == self.embedding_dim
    ...
    def add_documentation(self, documentation: str, **kwargs) -> str:
        entry_id = self._add_to_index(
            self.doc_index, self.doc_metadata, documentation, {"documentation": documentation}
        )
    ...
    def get_related_documentation(self, question: str, **kwargs) -> list:
        return [
            ...
            for metadata in self._get_similar(
                self.doc_index, self.doc_metadata, question, self.n_results_documentation
            )
        ]
    ...
    def remove_collection(self, collection_name: str) -> bool:
        if collection_name in ["sql", "ddl", "documentation"]:
            ...
            if self.curr_client == "persistent":
                self._save_index(
                    getattr(self, f"{collection_name}_index"), f"{collection_name}_index.faiss"
                )
