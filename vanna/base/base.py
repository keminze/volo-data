...
class VannaBase(ABC):
    ...
    def generate_sql(self, question: str, allow_llm_to_see_data=False, ddl_list=None, question_sql_list=None, **kwargs) -> str:
        ...
        if allow_llm_to_see_data:
            iteration = 0
            max_iterations = 5  # 防止死循环，可根据需要调整
            combined_markdown = ""
            ...
            try:
                while iteration < max_iterations:
                    iteration += 1
                    self.log(
                        title=f"Get Intermediate SQLs", message="Extracting intermediate SQLs..."
                    )

                    # 第 N 轮提取中间 SQL
                    intermediate_sqls = self.extract_intermediate_sqls(llm_response)
                    if not intermediate_sqls:
                        self.log(
                            title=f"Get Intermediate SQLs",
                            message="No more intermediate SQLs found.",
                        )
                        break
                    ...
                    for intermediate_sql in intermediate_sqls:
                        ...
                        dfs_markdown.append(f"\n{intermediate_sql}\n{df_markdown}\n")

                    combined_markdown += "\n".join(dfs_markdown)

                    # 将新的查询结果反馈给 LLM，让它生成新的 SQL 或最终答案
                    prompt = self.get_sql_prompt(
                        initial_prompt=initial_prompt,
                        question=(
                            f"The following are the results of the intermediate SQL queries:\n"
                            f"{combined_markdown}\n"
                    ...
                    self.log(title=f"SQL Prompt", message=prompt)
                    llm_response = self.submit_prompt(prompt, **kwargs)
                    self.log(title=f"LLM Response", message=llm_response)

                return self.extract_sql(llm_response)

            except Exception as e:
                return f"Error running intermediate SQLs: {e}"

        return self.extract_sql(llm_response)

    def extract_intermediate_sqls(self, llm_response: str) -> list[str]:
        """
        Extracts all SQL queries enclosed within <intermediate_sql>...</intermediate_sql> tags
        from the LLM response, and validates their syntax.

        Example:
        ```python
        vn.extract_intermediate_sqls(
            "<intermediate_sql>SELECT DISTINCT city FROM customers;</intermediate_sql>"
            "<intermediate_sql>SELECT DISTINCT category FROM products;</intermediate_sql>"
        )
        ```
        Output:
            ["SELECT DISTINCT city FROM customers;", "SELECT DISTINCT category FROM products;"]

        Args:
            llm_response (str): The full response from the LLM.

        Returns:
            list[str]: A list of valid SQL queries.
        """
        import re
        import sqlparse  # ✅ 用于语法解析验证

        # 提取所有 <intermediate_sql>...</intermediate_sql> 内的内容
        raw_sqls = re.findall(
            r"<intermediate_sql>(.*?)</intermediate_sql>",
            llm_response,
        ...
        for sql in raw_sqls:
            ...
            try:
                ...
                if not parsed or len(parsed) == 0:
                    self.log(title="Invalid SQL Skipped", message=f"{cleaned_sql}")
                    continue

                # 进一步检查是否为常见查询语句（SELECT、WITH、CREATE TABLE AS）
                first_token = parsed[0].token_first(skip_cm=True)
                if first_token and first_token.value.upper() in {"SELECT", "WITH", "CREATE"}:
                    valid_sqls.append(cleaned_sql)
                else:
                    self.log(title="Unexpected SQL Type Skipped", message=f"{cleaned_sql}")

            except Exception as e:
                self.log(title="SQL Parse Error", message=f"{cleaned_sql}\nError: {e}")

        if valid_sqls:
            self.log(title="Extracted Intermediate SQLs", message=f"{valid_sqls}")
        else:
            self.log(title="No Valid Intermediate SQLs Found", message=llm_response[:200])

        return valid_sqls

    def extract_sql(self, llm_response: str) -> str:
        """
        优先提取 <final_sql> 并对 SQL 进行清洗、格式化、语法校验。
        支持多种 LLM 输出格式。
        """
        import re
        import sqlparse
        ...
        def clean_sql(sql: str) -> str:
            ...
            # 去掉 <final_sql> 标签
            sql = re.sub(r"</?final_sql>", "", sql, flags=re.IGNORECASE).strip()
        ...
        def validate_sql(sql: str) -> bool:
            ...
            return first and first.value.upper() in {"SELECT", "WITH", "CREATE"}

        # --- 1️⃣ 先提取 <final_sql> ---
        m = re.search(r"<final_sql>(.*?)</final_sql>", llm_response, re.DOTALL | re.IGNORECASE)
        if m:
            sql = clean_sql(m.group(1))
            if validate_sql(sql):
                self.log(title="Extracted Final SQL", message=sql)
                return sql
        ...
        if m:
            sql = clean_sql(m.group(1))
            if validate_sql(sql):
                self.log(title="Extracted SQL from ```sql ... ```", message=sql)
                return sql

        # --- 3️⃣ 提取通用 ``` ... ``` ---
        m = re.search(r"```(.*?)```", llm_response, re.DOTALL | re.IGNORECASE)
        if m:
            sql = clean_sql(m.group(1))
            if validate_sql(sql):
                self.log(title="Extracted SQL from ``` ... ```", message=sql)
                return sql

        # --- 4️⃣ 回退到 SELECT / WITH / CREATE 模式匹配 ---
        m = re.search(
            r"(SELECT .*?;|WITH .*?;|CREATE .*?;)", llm_response, re.DOTALL | re.IGNORECASE
        )
        if m:
            sql = clean_sql(m.group(1))
            if validate_sql(sql):
                self.log(title="Extracted SQL from SELECT / WITH / CREATE", message=sql)
                return sql

        if validate_sql(llm_response):
            self.log(title="Extracted SQL from LLM Response", message=llm_response)
            return llm_response

        # --- ❗ 未找到 ---
        self.log(title="No SQL Extracted", message=llm_response)
        return ""
    ...
    def get_sql_prompt(
        ...
        **kwargs,
        ...
        if self.config.get("additional_prompt", None) is not None:
            logger.info("Adding additional prompt to SQL generation prompt.")
            initial_prompt += (
                "\n" + "=== Additional context ===" + self.config.get("additional_prompt") + "\n"
            )

        initial_prompt += f"""            
            === Query Generation Protocol (Agg-Lite Version) ===
            The conversation always proceeds in TWO phases:
            PHASE 1 = Intermediate enumeration phase (For data cleaning/filtering)
            PHASE 2 = Base Data SQL generation phase (For data extraction & coarse aggregation)
            You MUST NOT switch phases on your own. 
            You switch phases ONLY when the user explicitly provides additional information or confirms readiness.
            ---------------------------------------------------------
            PHASE 1 – Intermediate Enumeration Phase
            ---------------------------------------------------------
            You are in PHASE 1 *unless* the user explicitly says:
                "All distinct values are now provided"
                OR
                "继续生成 final SQL"
                OR
                "进入第二阶段"
            During PHASE 1:
            1. Scan ALL columns mentioned in the Context.
            2. For every column whose possible string values are NOT fully known, generate one `<intermediate_sql>` query:
                <intermediate_sql>
                SELECT DISTINCT column_name FROM table_name;
                </intermediate_sql>
            3. You MUST generate ALL required intermediate queries in ONE single response.
            4. After generating them, STOP.  
            Never repeat or regenerate intermediate queries in future responses.
            IMPORTANT:
            - If you already generated intermediate queries in a previous message, DO NOT generate them again.
            - If the user asks questions while you are still in PHASE 1, you only respond:
                "等待用户提供枚举结果，当前仍处于 PHASE 1。"
            and do NOT generate intermediate queries again.
            ---------------------------------------------------------
            PHASE 2 – Base Data SQL Generation Phase
            ---------------------------------------------------------
            You enter PHASE 2 ONLY after the user confirms distinct values or triggers "Final SQL".
            During PHASE 2, your goal is to generate a "Foundation Dataset" for downstream Python analysis.
            1. YOU MUST:
               - Perform essential filtering (WHERE/HAVING) based on user intent.
               - Perform COARSE aggregation only (e.g., GROUP BY main dimensions, SUM/COUNT of metrics).
               - Join necessary tables to pull required raw attributes.
               - Keep results at a "Analysis-Ready" grain (don't over-aggregate).

            2. YOU MUST NOT (Leave these to the Code Node):
               - DO NOT calculate complex ratios (e.g., conversion rate, YoY/MoM growth).
               - DO NOT use complex Window Functions (e.g., RANK, PARTITION BY).
               - DO NOT write multi-layer nested subqueries for business logic.
               - DO NOT perform complex CASE WHEN for data binning.

            3. The output must be exactly ONE `<final_sql>`.

            ---------------------------------------------------------
            COMPUTATION DELEGATION RULES (SQL vs CODE)
            ---------------------------------------------------------
            - SQL (Your Task): Fetch [Sales Amount], [Orders], [Date], [Category].
            - Code (Next Node): Calculate [Average Order Value], [YoY Growth %], [Sales Contribution %].

            ---------------------------------------------------------
            OUTPUT RULES
            ---------------------------------------------------------
            - NO backticks or markdown code fences.
            - Only `<intermediate_sql>` or `<final_sql>` blocks.
            - Never output both types in the same message.
            =========================================================
        """
    ...
    def connect_to_sqlite(self, url: str, check_same_thread: bool = False, **kwargs):
        ...
        path = os.path.join("uploaded_sqlite_dbs", os.path.basename(urlparse(url).path))
        ...
        if not os.path.exists(path):
            ...
            # url = path
        # print(f"Using SQLite database at {url}")
        # Connect to the database
        conn = sqlite3.connect(path, check_same_thread=check_same_thread, **kwargs)
        ...
        **Example:**
        ...
        try:
            conn = psycopg2.connect(
                host=host, dbname=dbname, user=user, password=password, port=port, **kwargs
            )
        ...
        **kwargs,
        ...
        try:
            conn = clickhouse_connect.get_client(
                host=host, port=port, username=user, password=password, database=dbname, **kwargs
            )
            print(conn)
        except Exception as e:
            raise ValidationError(e)
    ...
    def connect_to_oracle(self, user: str = None, password: str = None, dsn: str = None, **kwargs):
        """
    ...
    def connect_to_bigquery(self, cred_file_path: str = None, project_id: str = None, **kwargs):
        """
        ...
        **kwargs,
        ...
        try:
            conn = hive.Connection(
                host=host, username=user, password=password, database=dbname, port=port, auth=auth
            )
    ...
    def ask(
        ...
        **Example:**
        ...
        try:
            df = self.run_sql(sql)
            if df is None:
                self.log(title="No results returned from the database.", message=sql)
    ...
    def train(
        ...
        **Example:**
        ...
        if documentation:
            self.log(message=documentation, title="Adding documentation....")
            return self.add_documentation(documentation)

        if sql:
            if question is None:
                question = self.generate_question(sql)
                self.log(message="Question generated with sql:" + question, title="\nAdding SQL...")
            return self.add_question_sql(question=question, sql=sql)

        if ddl:
            self.log(title="Adding ddl:", message=ddl)
    ...
    def get_training_plan_generic(self, df) -> TrainingPlan:
        ...
        for database in df[database_column].unique().tolist():
            for schema in (
                df.query(f'{database_column} == "{database}"')[schema_column].unique().tolist()
            ):
                for table in (
                    df.query(
    ...
    def get_training_plan_snowflake(
            ...
            try:
                ...
                if filter_databases is not None:
                    mask = (
                        ...
                        .str.lower()
                        .apply(lambda x: any(s in x for s in [s.lower() for s in filter_databases]))
                    )
                ...
                if filter_schemas is not None:
                    mask = (
                        df_history_filtered["QUERY_TEXT"]
                        .str.lower()
                        .apply(lambda x: any(s in x for s in [s.lower() for s in filter_schemas]))
                    )
                    df_history_filtered = df_history_filtered[mask]
        ...
        for database in databases:
            ...
            try:
                ...
                df_columns = self.run_sql(f"SELECT * FROM {database}.INFORMATION_SCHEMA.COLUMNS")
