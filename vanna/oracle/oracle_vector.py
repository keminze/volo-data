...
class Oracle_VectorStore(VannaBase):
    def __init__(self, config=None):
        ...
        self.n_results_documentation = config.get("n_results_documentation", self.n_results)
            ...
            df_sql = pd.DataFrame(
                {
                    "id": ids,
                    "question": [
                        json.loads(doc)["question"] if isinstance(doc, str) else doc["question"]
    ...
    def update_training_data(self, id: str, train_type: str, question: str, **kwargs) -> bool:
        print(f"{train_type=}")
    ...
    def create_collections_if_not_exists(
        ...
        cmetadata = json.dumps(self.cmetadata) if cmetadata is None else json.dumps(cmetadata)
